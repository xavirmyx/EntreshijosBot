...
ping_respuestas = [
    "🏓 *¡Pong!* El bot está operativo y listo para ayudarte. 🌟",
    "🎾 *¡Pong!* Todo en orden, funcionando perfectamente. 💪",
    "🚀 *¡Pong!* El bot está activo y a pleno rendimiento. 🌍",
    "🎯 *¡Pong!* Respuesta inmediata, el bot está en línea. 🌟"
]
...
def handle_message(update, context):
    ...
    # Update the group title in the database with the latest title
    ...
    if is_valid_request:
        ...
        if chat_id not in CANALES_PETICIONES or thread_id != CANALES_PETICIONES[chat_id]["thread_id"]:
            notificacion = f"❌ {username_escaped}, las solicitudes solo son válidas en el canal correspondiente. 🌟"
            warn_message = f"/warn {username_escaped} Petición fuera del canal correcto."
        ...
        try:
            ...
            set_peticion_registrada(ticket_number, {
                ...
                "timestamp": timestamp,
                "chat_title": chat_title,  # Store the raw title with emojis
                "thread_id": thread_id
            })
        ...
        if user_data["count"] > 2 and user_id not in admin_ids:
            limite_message = f"❌ {username_escaped}, has alcanzado el límite de 2 peticiones diarias. Intenta mañana. 🌟"
        ...
        confirmacion_message = (
            ...
            f"📅 Fecha: {timestamp_str}\n"
            f"📝 Mensaje: {message_text_escaped}\n"
            f"🔍 Consulta el estado con: /estado {ticket_number}\n"
            "⏳ Será atendida pronto por nuestro equipo."
        )
        try:
        ...
        notificacion_incorrecta = (
            f"⚠️ {username_escaped}, utiliza únicamente: {', '.join(VALID_REQUEST_COMMANDS)}.\n"
        ...
        try:
            ...
            logger.info(f"⚠️ Notificación de petición incorrecta enviada a {username} en {chat_id}")
...
def handle_grupos(update, context):
    ...
    keyboard = [[InlineKeyboardButton("🔙 Volver", callback_data="grupos_retroceder")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def handle_historial(update, context):
    ...
    for row in solicitudes:
        ...
        historial.append(
            ...
            f"🏠 Grupo: {chat_title}\n"  # Use raw chat_title with emojis
    ...
    if len(historial_message) > 4096:
        parts = []
        current_part = "📜 *Historial de Solicitudes Gestionadas* 🌟\n\n"
        for line in historial:
            if len(current_part) + len(line) + 1 > 4096:
                parts.append(current_part)
                current_part = "📜 *Continuación del Historial* 🌟\n\n" + line
            else:
                current_part += "\n" + line
        if current_part:
            parts.append(current_part)
        for part in parts:
            try:
                ...
                bot.send_message(chat_id=chat_id, text=part.replace('*', ''))
    else:
        try:
...
def handle_go(update, context):
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("SELECT ticket_number, chat_id, username, chat_title FROM peticiones_registradas ORDER BY ticket_number")
...
def handle_eliminar(update, context):
    ...
    if not pendientes:
        ...
        return
    keyboard = [[InlineKeyboardButton(f"Ticket #{ticket} - {escape_markdown(username, True)}", callback_data=f"eliminar_{ticket}")] for ticket, username in pendientes]
    reply_markup = InlineKeyboardMarkup(keyboard)
    bot.send_message(chat_id=chat_id, text="🗑️ *Eliminar solicitud* 🌟\nSelecciona el ticket a eliminar:", reply_markup=reply_markup, parse_mode='Markdown')
...
def handle_subido(update, context):
    ...
    args = context.args
    if len(args) != 1:
        bot.send_message(chat_id=chat_id, text="❗ Uso correcto: /subido [ticket] 🌟", parse_mode='Markdown')
        return
    try:
        ticket = int(args[0])
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        bot.send_message(chat_id=chat_id, text=f"📋 *Confirmar acción* 🌟\n¿Marcar el Ticket #{ticket} de {escape_markdown(info['username'], True)} como subido?", reply_markup=reply_markup, parse_mode='Markdown')
    except ValueError:
        bot.send_message(chat_id=chat_id, text="❗ El ticket debe ser un número válido. 🌟", parse_mode='Markdown')
...
def handle_denegado(update, context):
    ...
    args = context.args
    if len(args) != 1:
        bot.send_message(chat_id=chat_id, text="❗ Uso correcto: /denegado [ticket] 🌟", parse_mode='Markdown')
        return
    try:
        ticket = int(args[0])
        info = get_peticion_registrada(ticket)
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        bot.send_message(chat_id=chat_id, text=f"📋 *Confirmar acción* 🌟\n¿Marcar el Ticket #{ticket} de {escape_markdown(info['username'], True)} como denegado?", reply_markup=reply_markup, parse_mode='Markdown')
    except ValueError:
        bot.send_message(chat_id=chat_id, text="❗ El ticket debe ser un número válido. 🌟", parse_mode='Markdown')
...
def handle_restar(update, context):
    ...
    if len(args) != 2 or not args[0].startswith('@'):
        bot.send_message(chat_id=chat_id, text="❗ Uso correcto: /restar @username [número] 🌟", parse_mode='Markdown')
        return
    username = args[0]
    try:
        amount = int(args[1])
        ...
        bot.send_message(chat_id=chat_id, text="❗ El segundo argumento debe ser un número válido. 🌟", parse_mode='Markdown')
        return
    with get_db_connection() as conn:
        c = conn.cursor()
        ...
        result = c.fetchone()
        if not result:
            bot.send_message(chat_id=chat_id, text=f"❌ {escape_markdown(username, True)} no encontrado en las peticiones. 🌟", parse_mode='Markdown')
            return
        ...
        conn.commit()
    bot.send_message(chat_id=chat_id, text=f"✅ Se han restado {amount} peticiones a {escape_markdown(username, True)}. Nuevo conteo: {new_count}/2 🌟", parse_mode='Markdown')
...
def handle_sumar(update, context):
    ...
    args = context.args
    if len(args) != 2 or not args[0].startswith('@'):
        bot.send_message(chat_id=chat_id, text="❗ Uso correcto: /sumar @username [número] 🌟", parse_mode='Markdown')
        return
    username = args[0]
    try:
        amount = int(args[1])
    ...
    with get_db_connection() as conn:
        ...
        c.execute("UPDATE peticiones_por_usuario SET count = %s WHERE user_id = %s", (new_count, user_id))
        conn.commit()
    bot.send_message(chat_id=chat_id, text=f"✅ Se han sumado {amount} peticiones a {escape_markdown(username, True)}. Nuevo conteo: {new_count}/2 🌟", parse_mode='Markdown')
...
def handle_graficas(update, context):
    ...
    try:
        ...
        # Log usernames to debug Markdown issues
        logger.info(f"📊 /graficas - admin_max_name: {admin_max_name}, user_max_name: {user_max_name}")

        # Enhanced escaping for usernames
        escaped_admin_max_name = admin_max_name if admin_max_name == "N/A" else escape_markdown(admin_max_name, preserve_username=True)
        escaped_user_max_name = user_max_name if user_max_name == "N/A" else escape_markdown(user_max_name, preserve_username=True)
...
def button_handler(update, context):
    ...
    if data == "grupos_retroceder":
        bot.send_message(chat_id=chat_id, text="📋 *Menú principal* 🌟\nUsa /menu para ver los comandos disponibles.", parse_mode='Markdown')
    ...
    if data.startswith("select_") or data.startswith("confirm_") or data.startswith("notify_") or data.startswith("back_"):
        ...
        if estado == "seleccion" and (data == "confirm_on" or data == "confirm_off"):
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("🔙 Volver", callback_data=f"back_{accion}")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            try:
        ...
        if estado == "confirmacion" and data.startswith("notify_"):
            ...
            if decision == "yes":
                mensaje = (
                    "✅ *Solicitudes reactivadas* 🌟\nYa puedes enviar solicitudes (máximo 2 por día). ¡Gracias por tu paciencia!"
                ) if accion == "on" else (
                    "❌ *Solicitudes desactivadas* 🌟\nNo se aceptan nuevas solicitudes hasta nuevo aviso. Disculpa las molestias."
                )
                ...
                texto = f"{'🟢' if accion == 'on' else '🔴'} *Solicitudes {'activadas' if accion == 'on' else 'desactivadas'} y notificadas con éxito.* 🌟"
                try:
    ...
    if data.startswith("pend_"):
        if data == "pend_regresar":
            with get_db_connection() as conn:
                ...
                pendientes = c.fetchall()
            keyboard = [[InlineKeyboardButton(f"#{ticket} - {escape_markdown(username, True)} ({chat_title})",
        ...
        if len(data.split("_")) == 2:
            keyboard = [
                ...
                [InlineKeyboardButton("🔙 Volver", callback_data="pend_regresar")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            texto = (
                f"📋 *Solicitud #{ticket}* 🌟\n"
        ...
        if accion in ["subido", "denegado", "eliminar"] and len(data.split("_")) == 3:
            ...
            texto = f"📋 *Confirmar acción* 🌟\n¿Marcar el Ticket #{ticket} de {escape_markdown(info['username'], True)} como {accion_str}?"
            try:
                query.edit_message_text(text=texto, reply_markup=reply_markup, parse_mode='Markdown')
            except telegram.error.BadRequest:
                query.edit_message_text(text=texto.replace('*', ''), reply_markup=reply_markup)
        ...
        if len(data.split("_")) == 4 and data.endswith("confirm"):
            ...
            if accion == "subido":
                notificacion = f"✅ {username_escaped}, tu solicitud (Ticket #{ticket}) \"{message_text_escaped}\" ha sido aceptada y subida. 🌟"
                try:
                ...
                try:
                    ...
                    bot.send_message(chat_id=chat_id, text=f"✅ Ticket #{ticket} de {username_escaped} eliminado con éxito. 🌟", parse_mode='Markdown')
                except telegram.error.TelegramError as e:
    ...
    if data.startswith("confirm_subido_"):
        ...
        try:
            bot.send_message(chat_id=info["chat_id"], text=f"✅ {escape_markdown(info['username'], True)}, tu solicitud (Ticket #{ticket}) ha sido aceptada y subida. 🌟", parse_mode='Markdown', message_thread_id=info.get("thread_id"))
        except telegram.error.BadRequest:
            bot.send_message(chat_id=info["chat_id"], text=f"✅ {escape_markdown(info['username'], True)}, tu solicitud (Ticket #{ticket}) ha sido aceptada y subida. 🌟".replace('*', ''), message_thread_id=info.get("thread_id"))
        try:
