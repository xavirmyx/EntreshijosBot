...
admin_ids = set([12345678])  # Actualiza con IDs reales de administradores
GRUPOS_PREDEFINIDOS = {
...
def safe_bot_method(bot_method, *args, **kwargs):
    try:
        return bot_method(*args, **kwargs)
    except telegram.error.Unauthorized:
        logger.warning(f"Bot no tiene permisos para {bot_method.__name__}")
        return None
    except telegram.error.TelegramError as e:
        logger.error(f"Error de Telegram en {bot_method.__name__}: {str(e)}")
        return None
...
def handle_message(update, context):
    ...
    if is_valid_request:
        ...
        if not sent_message:
            sent_message = safe_bot_method(bot.send_message, chat_id=GROUP_DESTINO, text=destino_message.replace('*', ''))
        if sent_message:
            ...
            safe_bot_method(bot.edit_message_text, chat_id=GROUP_DESTINO, message_id=sent_message.message_id, text=destino_message, parse_mode='Markdown') or \
            safe_bot_method(bot.edit_message_text, chat_id=GROUP_DESTINO, message_id=sent_message.message_id, text=destino_message.replace('*', ''))
            ...
            safe_bot_method(bot.send_message, chat_id=canal_info["chat_id"], text=confirmacion_message, parse_mode='Markdown', message_thread_id=canal_info["thread_id"]) or \
            safe_bot_method(bot.send_message, chat_id=canal_info["chat_id"], text=confirmacion_message.replace('*', ''), message_thread_id=canal_info["thread_id"])
        ...
        warn_message = f"/warn {username_escaped} Petición mal formulada" if len(intentos_recientes) <= 2 else \
                       f"/warn {username_escaped} Abuso de peticiones mal formuladas"
...
def handle_sumar_command(update, context):
    ...
    try:
        ...
        safe_bot_method(bot.send_message, chat_id=chat_id, text="❗ El número debe ser un entero. 🌟", parse_mode='Markdown')
        return
    ...
    user_data = get_peticiones_por_usuario(user_id)
    if not user_data:
        set_peticiones_por_usuario(user_id, amount, chat_id, username)
        safe_bot_method(bot.send_message, chat_id=chat_id, text=f"✅ Se han sumado {amount} peticiones a {username}. 🌟", parse_mode='Markdown')
    else:
        new_count = user_data['count'] + amount
        set_peticiones_por_usuario(user_id, new_count, user_data['chat_id'], user_data['username'])
        safe_bot_method(bot.send_message, chat_id=chat_id, text=f"✅ Se han sumado {amount} peticiones a {username}. Nuevo total: {new_count}/2 🌟", parse_mode='Markdown')
...
def handle_graficas(update, context):
    ...
    stats_msg = (
        ...
        f"🚫 Límite exced ContentTooLongexcedido: {stats.get('limite_excedido', 0)}"
    )
...
def button_handler(update, context):
    ...
    if data == "menu_pendientes":
        ...
        try:
            ...
            safe_bot_method(bot.send_message, chat_id=chat_id, text=message_text, reply_markup=reply_markup, parse_mode='Markdown') or \
            safe_bot_method(bot.send_message, chat_id=chat_id, text=message_text.replace('*', ''), reply_markup=reply_markup)
            safe_bot_method(query.message.delete)
        except Exception as e:
            logger.error(f"Error en menu_pendientes: {str(e)}")
            safe_bot_method(bot.send_message, chat_id=chat_id, text="❌ Error al mostrar pendientes. Intenta de nuevo. 🌟", parse_mode='Markdown')
            safe_bot_method(query.message.delete)
        finally:
            if conn and not conn.closed:
                conn.close()
    ...
    if data == "menu_historial":
        ...
        try:
            ...
            if not solicitudes:
                ...
                return
            historial = []
            for row in solicitudes[:5]:  # Limitar a 5 para evitar mensajes largos
    ...
    if data.startswith("select_") or data.startswith("confirm_") or data.startswith("notify_") or data.startswith("back_"):
        ...
        if estado == "seleccion" and (data.startswith("select_on_") or data.startswith("select_off_")):
            ...
            keyboard = [[InlineKeyboardButton(f"{info['title']} {'🟢' if info['activo'] else '🔴'}{' ✅' if gid in grupos_seleccionados[chat_id]['grupos'] else ''}",
                                            callback_data=f"select_{accion}_{gid}")] for gid, info in grupos_estados.items() if str(gid) != GROUP_DESTINO]
    ...
    if data.startswith("pend_"):
        ...
        if data.endswith("_notify_yes") or data.endswith("_notify_no"):  # Manejar notificación
            ...
            if notify:
                notification_text = ""
                if accion == "subido":
                    notification_text = f"✅ {username_escaped}, tu solicitud (Ticket #{ticket}) \"{message_text_escaped}\" ha sido subida. 🎉"
                elif accion == "denegado":
                    notification_text = f"❌ {username_escaped}, tu solicitud (Ticket #{ticket}) \"{message_text_escaped}\" ha sido denegada. 🌟"
                elif accion == "eliminar":
                    notification_text = f"ℹ️ {username_escaped}, tu solicitud (Ticket #{ticket}) \"{message_text_escaped}\" ha sido eliminada. 🌟"

                if safe_bot_method(bot.send_message, chat_id=info["chat_id"], text=notification_text, 
                                 parse_mode='Markdown', message_thread_id=info.get("thread_id")) and accion == "eliminar":
...
if __name__ == '__main__':
    ...
    safe_bot_method(bot.set_webhook, url=f"{WEBHOOK_URL}/webhook")
